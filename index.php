...
if ($_POST) {
	foreach ($_POST as $guess) {
		if ($guess == "") {
			break;
		} elseif (ucwords($guess) == "Tina Turner") {
		...
		if ($xml->error) {
			$output .= "<p>Sorry, couldn't find any matches for \"".ucwords($guess)."\"!</p><p>There might be some ".ucwords($guess)." content elsewhere on heart.co.uk. <a target=\"_blank\" href=\"http://www.heart.co.uk/search/?q=".$guess."\">Search...</a></p>";
		} else {
